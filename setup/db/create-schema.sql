...
CREATE TABLE  `cloud`.`domain` (
  ...
  `display_name` varchar(255) COMMENT 'display name of the domain shown on the UI, falls back to name when not set',
  `sort_key` int unsigned NOT NULL DEFAULT 0 COMMENT 'sort order of the domain on the login page',
  `show_on_login` boolean NOT NULL DEFAULT true COMMENT 'whether the domain is listed on the login page domain selector',
