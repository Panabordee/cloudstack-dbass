...
# and the same `mysql` client, and this script touches no service-specific
# config paths.
#
# Resets an existing user's password. Unlike mariadb.sh this must find the user
...
# Same discipline as mariadb.sh: prove the new credential authenticates before
