    ...
    <a-spin :spinning="loading">
      <a-form
        ...
        <a-form-item name="title" ref="title">
          ...
          <a-input v-model:value="form.title" :placeholder="apiParams.title.description" v-focus="true" />
        </a-form-item>
...
export default {
  ...
  beforeCreate () {
    this.apiParams = this.$getApiParams('updateAnnouncement')
  },
  created () {
    this.initForm()
  },
  ...
  methods: {
    initForm () {
      ...
      this.form = reactive({
        ...
        type: resource.type || 'info',
        message: resource.message,
        enabled: resource.enabled === true || resource.enabled === 'true',
        closable: resource.closable !== false && resource.closable !== 'false',
        persistdismissal: resource.persistdismissal !== false && resource.persistdismissal !== 'false',
        startdate: resource.startdate ? dayjs(resource.startdate) : null,
        enddate: resource.enddate ? dayjs(resource.enddate) : null,
    ...
    handleSubmit (e) {
      ...
      this.formRef.value.validate().then(() => {
        ...
        this.loading = true
        postAPI('updateAnnouncement', params).then(json => {
          this.$emit('refresh-data')
          this.$notification.success({
            message: this.$t('label.edit.announcement'),
            description: `${this.$t('message.success.update.announcement')} ${params.title}`
          })
          this.closeAction()
        }).catch(error => {
